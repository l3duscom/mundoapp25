...
class Checkout extends BaseController
{
	...
	public function finalizarcartaoOLD2()
	{
		...
		$evento = $this->eventoModel->select()->where('id', 12)->first();
	...
	public function finalizarcartaoold3()
	{
		helper('text');
		$retorno['token'] = csrf_hash();
		...
		try {
			...
			$pedido_id = $this->criaPedidoCartao($post, $user_id);
	...
	public function finalizarCartao()
	{
		try {
			$post = $this->request->getPost();
			$retorno = $this->checkoutService->processarPagamentoCartao($post);

			if ($retorno['status'] === 'ok') {
				return redirect()->to(site_url("checkout/obrigado"));
			}

			return redirect()->to(site_url("checkout/cartao"))
				->with('erro', $retorno['erro'] ?? 'Erro ao processar o pagamento');
		} catch (\Throwable $e) {
			log_message('error', 'Erro em finalizarCartao: ' . $e->getMessage());
			return redirect()->to(site_url("checkout/cartao"))
				->with('erro', 'Erro inesperado ao processar o pagamento com cartão.');
		}
	}

	private function criaPedidoCartao(array $post, int $user_id): int
	{
	...
	public function finalizarpixold2()
	{
		if (! $this->request->isAJAX()) {
			return redirect()->back();
		}
		...
		try {
			...
			$pedido_id = $this->criaPedido($post, $user_id);
			$this->registraIngressos($pedido_id, $user_id);
	...
	public function finalizarPix()
	{
		...
		try {
			...
			$retorno = $this->checkoutService->processarPagamentoPix($post);
			return $this->response->setJSON($retorno);
		} catch (\Throwable $e) {
			log_message('error', 'Erro em finalizarPix: ' . $e->getMessage());
			return $this->response->setJSON([
				'erro' => 'Erro inesperado ao processar o pagamento via PIX.'
			]);
		}
	}
	...
	private function criaPedido(array $post, int $user_id): int
	{
		$frete = ($post['frete'] ?? '') === 'casa' ? 1 : 0;
