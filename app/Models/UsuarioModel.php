...
class UsuarioModel extends Model
{
    ...
    protected $beforeInsert         = ['hashPassword', 'generateCodigo'];
    ...
    public function buscaUsuarioParaRedefinirSenha(string $token)
    {
        ...
        // Se reset_expira_em for null, o link não expira
        if ($usuario->reset_expira_em !== null && $usuario->reset_expira_em < date('Y-m-d H:i:s')) {
            return null;
        }
    ...
    public function recuperaAtendentesParaRelatorio(string $dataInicial, string $dataFinal)
    {
        ...
        $where = 'ordens.created_at BETWEEN "' . $dataInicial . '" AND "' . $dataFinal . '"';
    ...
    protected function generateCodigo(array $data): array
    {
        if (isset($data['data']['nome'])) {
            $data['data']['codigo'] = strtoupper(random_string('alnum', 7));
        }
        return $data;
    }

    public function geraCodigoExtra(): string
    {
        do {
            $codigo = strtoupper(random_string('alnum', 7));
    ...
    public function geraCodigoUsuario(): string
    {
        do {
            $codigo = strtoupper(random_string('alnum', 6));

            $this->select('password')->where('password', $codigo);
        } while ($this->countAllResults() > 1);
